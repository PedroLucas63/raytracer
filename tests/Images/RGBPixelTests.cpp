...
TEST_CASE("RGBPixel default constructor initializes a black pixel") {
   const raytracer::RGBPixel pixel {};

   requirePixel(pixel, 0, 0, 0);
}

TEST_CASE("RGBPixel constructor stores the informed channels") {
   const raytracer::RGBPixel pixel {12, 34, 56};

   requirePixel(pixel, 12, 34, 56);
}

TEST_CASE("RGBPixel setters update channels independently") {
   raytracer::RGBPixel pixel {};
...
TEST_CASE("RGBPixel channel operator returns the selected channel") {
...
TEST_CASE("RGBPixel channel operator rejects invalid channels") {
   const raytracer::RGBPixel pixel {10, 20, 30};
...
TEST_CASE("RGBPixel predefined constants expose the expected colors") {
...
TEST_CASE("RGBPixel addition sums channels and saturates at 255") {
   const raytracer::RGBPixel left {100, 120, 140};
   const raytracer::RGBPixel right {10, 20, 30};
   const raytracer::RGBPixel saturatedLeft {250, 240, 230};
   const raytracer::RGBPixel saturatedRight {10, 20, 30};
...
TEST_CASE("RGBPixel compound addition mutates the pixel and returns itself") {
   raytracer::RGBPixel pixel {250, 240, 230};

   auto& result = (pixel += raytracer::RGBPixel {10, 20, 30});

   REQUIRE(&result == &pixel);
   requirePixel(pixel, 255, 255, 255);
}

TEST_CASE("RGBPixel subtraction subtracts channels and saturates at zero") {
   const raytracer::RGBPixel left {100, 120, 140};
   const raytracer::RGBPixel right {10, 20, 30};
   const raytracer::RGBPixel saturatedLeft {5, 10, 15};
   const raytracer::RGBPixel saturatedRight {10, 20, 30};
...
TEST_CASE("RGBPixel compound subtraction mutates the pixel and returns itself") {
   raytracer::RGBPixel pixel {5, 10, 15};
...
TEST_CASE("RGBPixel multiplication scales channels with rounding and saturation") {
   const raytracer::RGBPixel pixel {5, 15, 25};
   const raytracer::RGBPixel saturatedPixel {200, 220, 240};
...
TEST_CASE("RGBPixel compound multiplication mutates the pixel and returns itself") {
   raytracer::RGBPixel pixel {10, 20, 30};
...
TEST_CASE("RGBPixel multiplication rejects negative scales") {
   const raytracer::RGBPixel pixel {10, 20, 30};
...
TEST_CASE("RGBPixel division scales channels down with rounding") {
   const raytracer::RGBPixel pixel {10, 21, 31};

   requirePixel(pixel / 2.0, 5, 11, 16);
   requirePixel(pixel, 10, 21, 31);
}

TEST_CASE("RGBPixel compound division mutates the pixel and returns itself") {
   raytracer::RGBPixel pixel {10, 21, 31};
...
TEST_CASE("RGBPixel division rejects non-positive scales") {
   const raytracer::RGBPixel pixel {10, 20, 30};

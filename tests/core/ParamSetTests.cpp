...
TEST_CASE("ParamSet stores and resolves 4-color background from XML attributes") {
   ...
   REQUIRE(ps.resolve<std::string>("type") == "4_colors");
 
   requireColor(ps.resolve<raytracer::RGBColor>("bl"),   0,   0,  51);
   requireColor(ps.resolve<raytracer::RGBColor>("tl"),   0, 255,  51);
   requireColor(ps.resolve<raytracer::RGBColor>("tr"), 255, 255,  51);
   requireColor(ps.resolve<raytracer::RGBColor>("br"), 255,   0,  51);
}
...
TEST_CASE("ParamSet resolves correct corner after overwriting a color") {
   ...
   requireColor(ps.resolve<raytracer::RGBColor>("tl"), 10, 20, 30);
}
...
TEST_CASE("ParamSet throws when resolving a color key as wrong type") {
   ...
   REQUIRE_THROWS_AS(ps.resolve<std::string>("bl"), std::bad_cast);
}
...
TEST_CASE("ParamSet correctly distinguishes all four corner keys") {
   ...
   requireColor(ps.resolve<raytracer::RGBColor>("tl"), 10, 10, 10);
   requireColor(ps.resolve<raytracer::RGBColor>("tr"), 20, 20, 20);
   requireColor(ps.resolve<raytracer::RGBColor>("bl"), 30, 30, 30);
   requireColor(ps.resolve<raytracer::RGBColor>("br"), 40, 40, 40);
}
...
TEST_CASE("ParamSet stores and resolves solid background") {
   ...
   REQUIRE(ps.resolve<std::string>("type") == "solid");
   requireColor(ps.resolve<raytracer::RGBColor>("color"), 100, 150, 200);
}
...
TEST_CASE("ParamSet stores and resolves horizontal gradient background") {
   ...
   REQUIRE(ps.resolve<std::string>("type") == "horizontal_gradient");
   requireColor(ps.resolve<raytracer::RGBColor>("from"),   0,   0,   0);
   requireColor(ps.resolve<raytracer::RGBColor>("to"),   255, 255, 255);
}
...
TEST_CASE("ParamSet resolve returns the value that was added") {
   ...
   REQUIRE(ps.resolve<int>("width") == 800);
}
...
TEST_CASE("ParamSet resolve works for multiple types") {
   ...
   REQUIRE(ps.resolve<int>("samples")         == 16);
   REQUIRE(ps.resolve<float>("fov")           == 45.0f);
   REQUIRE(ps.resolve<std::string>("name")    == "scene");
}
...
TEST_CASE("ParamSet add overwrites a previously stored value") {
   ...
   REQUIRE(ps.resolve<int>("depth") == 8);
}
...
TEST_CASE("ParamSet resolve throws out_of_range for missing key") {
   ...
   REQUIRE_THROWS_AS(ps.resolve<int>("missing"), std::out_of_range);
}
...
TEST_CASE("ParamSet resolve throws bad_cast when type does not match") {
   ...
   REQUIRE_THROWS_AS(ps.resolve<float>("value"), std::bad_cast);
}
...
TEST_CASE("ParamSet remove makes key unreachable via resolve") {
   ...
   REQUIRE_THROWS_AS(ps.resolve<int>("temp"), std::out_of_range);
}
